...
#extracting coordinates as (2x2) matrix , from the labeling files then storing in dicts
d_rect={}
d_square={}
...
class Block:
    def __init__(self , name , image , lb_file , coord):
        ...
        self.coord = coord
        self.size = np.array(self.coord[1]-self.coord[0])
        self.old_center = np.array( self.coord[1] - (self.size/2) )
        self.new_coord = None
        
    #methods to manipulate blocks 
    def getCenter (self):
        l=self.coord
        c = np.array( self.coord[1] - (self.size/2) )
        return c
...
for elm in d_square:
    ...
    copy.paste(crp , place)
    copy.show()'''
>>>>>>> 2b749d3219bc83a866262d84482a86a86f32d64c
